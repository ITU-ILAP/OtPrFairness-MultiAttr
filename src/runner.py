...
class RecRunner_FOCF_AbsUnf(RecRunner):
    ...
    def fit(self, model, batches, epoch=-1):  # fit the results for an input set
        ...
        for batch in tqdm(batches, leave=False, desc='Epoch %5d' % (epoch + 1),
            ...
            group_0_u_mask = torch.Tensor([1 if uid.item() in self.group_0_ids else 0 for uid in uids])
            group_1_u_mask = torch.Tensor([1 if uid.item() in self.group_1_ids else 0 for uid in uids])
            if torch.cuda.device_count() > 0:
                group_0_u_mask = group_0_u_mask.cuda()
                group_1_u_mask = group_1_u_mask.cuda()
...
class RecRunner_FairPO(RecRunner_PCFR):
    """
    FairPO Runner: Fair Process and Outcome.

    Combines two complementary fairness signals in a single training objective:
      1. Process fairness  (adversarial, from PCFR):
            L_adv  = -sum_k  discriminator_k(filtered_u, sensitive_labels_k)
         The model is penalised for making its filtered user embeddings
         predictive of sensitive attributes.

      2. Outcome fairness  (value-unfairness regularisation, from FOCF_ValUnf):
            mu_g   = mean(prediction_g - label_g)    for g in {group_0, group_1}
            L_out  = smooth_l1(|mu_0 - mu_1|, 0) * (batch_size / 2)
         The model is penalised for a directional prediction-bias gap between groups.

    Total loss:
        L = L_rec + fairpo_alpha * L_adv + fairpo_beta * L_out

    New CLI arguments:
        --fairpo_alpha  (default 1.0)  weight for adversarial process-fairness penalty
        --fairpo_beta   (default 1.0)  weight for outcome-fairness regularisation term
    """

    @staticmethod
    def parse_runner_args(parser):
        parser = RecRunner.parse_runner_args(parser)
        parser.add_argument('--fairpo_alpha', type=float, default=1.0,
                            help='Weight for adversarial process-fairness penalty in FairPO.')
        parser.add_argument('--fairpo_beta', type=float, default=1.0,
                            help='Weight for outcome-fairness (value-unfairness) penalty in FairPO.')
        return parser

    def __init__(self, group_0_ids, group_1_ids,
                 fairpo_alpha=1.0, fairpo_beta=1.0,
                 optimizer='GD', learning_rate=0.01, epoch=100,
                 batch_size=128, eval_batch_size=128 * 128,
                 dropout=0.2, l2=1e-5, metrics='RMSE',
                 disc_metrics='auc,acc,f1_macro,f1_micro',
                 check_epoch=10, early_stop=1, num_worker=0, disc_epoch=1000):
        RecRunner.__init__(self, optimizer, learning_rate, epoch, batch_size,
                           eval_batch_size, dropout, l2, metrics, disc_metrics,
                           check_epoch, early_stop, num_worker, disc_epoch)
        self.group_0_ids = group_0_ids
        self.group_1_ids = group_1_ids
        self.fairpo_alpha = fairpo_alpha
        self.fairpo_beta = fairpo_beta

    def fit(self, model, batches, fair_disc_dict, epoch=-1):
        """
        One training epoch for FairPO.
        Builds on PCFR's adversarial update and adds a group-aware
        value-unfairness regularisation term.
        """
        gc.collect()
        torch.cuda.empty_cache()
        ...
        for batch in tqdm(batches, leave=False, desc='Epoch %5d' % (epoch + 1),
            ...
            mask = np.asarray([1] * model.num_features)
            labels = batch['features'][:len(batch['features']) // 2, :]
            ...
            # Process-fairness penalty (adversarial, same as PCFR)
            fair_d_penalty = 0
            for fair_disc, label in masked_disc_label:
                fair_d_penalty += fair_disc(vectors, label)
            fair_d_penalty *= -1   # model wants to fool the discriminator

            # Outcome-fairness penalty (value-unfairness regularisation)
            uids = batch['X'][:, 0]
            val_loss = result_dict['val_loss']

            use_cuda = torch.cuda.device_count() > 0
            g0_mask = torch.tensor(
                [1.0 if uid.item() in self.group_0_ids else 0.0 for uid in uids])
            g1_mask = torch.tensor(
                [1.0 if uid.item() in self.group_1_ids else 0.0 for uid in uids])
            if use_cuda:
                g0_mask = g0_mask.cuda()
                g1_mask = g1_mask.cuda()

            eps = 1e-10
            fair_out_input = torch.abs(
                torch.sum(val_loss * g0_mask) / (torch.sum(g0_mask) + eps) -
                torch.sum(val_loss * g1_mask) / (torch.sum(g1_mask) + eps)
            )
            fair_out_target = torch.zeros_like(fair_out_input)
            fair_out_loss = smooth_l1_loss(fair_out_input, fair_out_target) * (len(batch['X']) // 2)

            # Combined loss
            loss = rec_loss + self.fairpo_alpha * fair_d_penalty + self.fairpo_beta * fair_out_loss
            loss.backward()
            model.optimizer.step()
            ...
            # Discriminator update (identical to PCFR)
            if len(masked_disc_label) != 0:
                for discriminator, label in masked_disc_label:
                    discriminator.optimizer.zero_grad()
                    disc_loss = discriminator(vectors.detach(), label)
            ...
            # Collect discriminator eval results
            if eval_dict is None:
                eval_dict = self._eval_discriminator(
                    model, labels, vectors.detach(), fair_disc_dict, len(mask))
            else:
                ...
                for f_name in eval_dict:
                    eval_dict[f_name]['label'] = torch.cat(
                        (eval_dict[f_name]['label'], batch_eval_dict[f_name]['label']), dim=0)
                    eval_dict[f_name]['prediction'] = torch.cat(
                        (eval_dict[f_name]['prediction'], batch_eval_dict[f_name]['prediction']), dim=0)

        # Aggregate discriminator scores
        d_score_dict = {}
        if eval_dict is not None:
            for f_name in eval_dict:
                l = eval_dict[f_name]['label']
        ...
        output_dict['loss'] = np.mean(loss_list)
        return output_dict
    # train() and fit_disc() are inherited from RecRunner_PCFR unchanged.
